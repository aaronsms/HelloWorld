  ...
  Stream<RegisterProfileState> mapEventToState(
    ...
    yield* event.map(
      ...
      locationDeleted: (event) async* {
        final currList = state.preferredLocations;

        yield state.copyWith(isChangingItem: true);
        currList.removeWhere((element) => element.value1 == event.latLng);
        yield state.copyWith(isChangingItem: false);
      },
      locationAdded: (event) async* {
        final location = event.location;
        final currList = state.preferredLocations;

        yield state.copyWith(isChangingItem: true);
        currList.add(Tuple2(location.latLng, Location(location.address)));
        yield state.copyWith(isChangingItem: false);
      },
    );
  }
